...
GESTURES = {'tho_data_cleaned': 0, 'ngungtho_data_cleaned': 1}
...
for i in range(len(x)-SAMPLES_PER_GESTURE):
    temp = []
    for j in range(i, i+SAMPLES_PER_GESTURE):
        temp.append(x[j])
    modDataset.append(temp)

for i in range(len(y)-SAMPLES_PER_GESTURE):
    temp = []
    for j in range(i, i+SAMPLES_PER_GESTURE):
        temp.append(y[j])
...
modDataset = np.array(modDataset).reshape(-1, SAMPLES_PER_GESTURE, SENSOR_NUM)
