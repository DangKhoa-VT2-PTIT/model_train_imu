...
float concatenated_data[resp_samples] = {};  // Mảng kết hợp từ ax, ay, az
...
void taskReadSensor(void *parameter) {
  ...
  while (1) {
    ...
    for (int i = 0; i < resp_samples; i++) {
      // Đọc dữ liệu từ các trục ax, ay, az
      int16_t ax, ay, az;
      mpu.getAcceleration(&ax, &ay, &az);

      resp_data[i * (num_features/2)] = (float)ax / 16384.0; // Chuyển đổi đơn vị sang g
